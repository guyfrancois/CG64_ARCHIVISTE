package archiviste.view
{
	...
	import archiviste.model.OG_Contextualiser;
	import archiviste.model.OG_ContextualiserDoc;
	import archiviste.model.OG_ContextualiserSeries;
	 ...
	 *
	 * archiviste.view.JeuContextualiserView
	 * libZoneJeu_contextualiser
	 */
	public class JeuContextualiserView extends Sprite implements I_GameModelView
	{
		...
		/*-----------------------------------*/
		private var v_docsSeries:Vector.<JeuContextualiserSerieView>;
		private var model : OG_Contextualiser;
		...
		public function JeuContextualiserView()
		{
			super();
		...
		public function setModel(model:OG_ABS):void {	
			this.model=model as OG_Contextualiser;
		}
		...
		private function init():void{
			if (isInit) return;
			isInit=true;
			v_docsSeries = new Vector.<JeuContextualiserSerieView>(model.v_doc.length);
		...
		protected function evt_selected_vignette(event:Event=null):void
		{
			...
			var sItem:JeuContextualiserSerieView;
			if (v_docsSeries[vignettes.vSelected]) {
				sItem = v_docsSeries[vignettes.vSelected];
			} else {
				sItem = v_docsSeries[vignettes.vSelected]= buildSerieContextualiserView();
			...
			sItem.addEventListener(JeuContextualiserSerieView.GAMECOMPLETE,evt_docComplete,false,0,true);
			sItem.addEventListener(JeuContextualiserSerieView.GAMEINCOMPLETE,evt_docInComplete,false,0,true);
			docZoom.blur=model.v_doc[vignettes.vSelected].blur;
			serieContainer.addChild(sItem);
		}
		
		protected function evt_docInComplete(event:Event):void	{	
			docZoom.zoomEnable=false;
		...
		private function buildSerieContextualiserView():JeuContextualiserSerieView {
			var cl:Class=Clips.classFromLib("lib_sys_serie_contextualiser",GameController.instance.main.loaderInfo.applicationDomain);
			var item:JeuContextualiserSerieView= new  cl() as JeuContextualiserSerieView;
			return item;
		}
	}
}